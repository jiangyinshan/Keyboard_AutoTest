...
class en_swipeTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_start(self):
        ...
        xls = excel(xls_road)
        desire_words = xls.getCellValue(5, 3)
        print("英语滑行输入测试开始")
        try:
            ...
            xls.setCellValue(5, 5, message)  # 实际上屏内容
            if len(message) <= 0:
                xls.setCellValue(5, 4, "输入框内容为空，测试结果错误")  # 保存测试结论
                time.sleep(1)
            elif message == desire_words:  # 将输入框内容message转换成小写字母和tag对比
                xls.setCellValue(5, 4, "滑动输入上屏成功，内容正确")  # 保存测试结论
                time.sleep(1)
            elif len(message) > 0 and message != desire_words:
                xls.setCellValue(5, 4, "滑动输入上屏成功，内容错误")  # 保存测试结论
                time.sleep(1)
...
if __name__ == '__main__':
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(en_swipeTest)
