...
class emoji_click_inputTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_start(self):
        ...
        xls = excel(xls_road)
        words = xls.getCellValue(10, 2)
        desire_words = xls.getCellValue(10, 3)
        print(words)
        print("emoji点击上屏测试开始")
        try:
            util.keyboard_show(self)
            util.change_emoji_style(self)
            samsung6_inputkey.emoji(self, 0)
            time.sleep(1)
            ...
            xls.setCellValue(10, 5, message)  # 保存实际上屏内容
            if len(message) <= 0:
                xls.setCellValue(10, 4, "输入框内容为空，测试结果错误")  # 保存测试结论
                time.sleep(1)
            elif message == desire_words:  # 将输入框内容message转换成小写字母和tag对比
                xls.setCellValue(10, 4, "上屏成功，输入内容正确")
                time.sleep(1)
            elif len(message) > 0 and message != desire_words:
                xls.setCellValue(10, 4, "上屏成功，输入内容错误")
                time.sleep(1)
...
if __name__ == '__main__':
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(emoji_click_inputTest)
